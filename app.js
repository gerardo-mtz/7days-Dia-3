...
const areaDeDesarrollo = parseInt(
  prompt("¿Te gustaria desarrollarte en \n1. Front-End \n2. Back-End?")
);
...
if (areaDeDesarrollo !== 1 && areaDeDesarrollo !== 2) {
  alert(
    "Respuesta no válida. Debes elegir 1 para Front-End o 2 para Back-End."
  );
  ...
  const eleccionFramework = parseInt(
    prompt("¿Te gustaria aprender \n1. React o \n2. Vue?")
  );
...
alert(
  `Elegiste aprender ${
    areaDeDesarrollo === 1 ? "Front-End" : "Back-End"
  } con ${framework}.`
);
...
if (areaDeDesarrollo === 1) {
  especializacion = parseInt(
    prompt("En que te gustaria especializarte? \n1. Front-End \n2. Full-Stack")
  );
  alert(
    `Elegiste especializarte en ${
      especializacion === 1 ? "Front-End" : "Full-Stack"
    }`
  );
} else if (areaDeDesarrollo === 2) {
  especializacion = parseInt(
  ...
  alert(
    `Elegiste especializarte en ${
      especializacion === 1 ? "Back-End" : "Full-Stack"
    }`
  );
}
...
while (continuar) {
  ...
  const agregarTecnologia = parseInt(
    prompt("¿Te gustaria aprender otra teconologia? \n1. Si \n2. No")
  );
...
if (aprenderTecnologias.length > 0) {
  alert(
    `Deseas aprender las siguientes tecnologias: ${aprenderTecnologias.join(
      ", "
    )}`
  );
} else {
